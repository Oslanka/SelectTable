...
import com.cloud.lashou.widget.banner.listener.OnBannerClickListener;
...
public class Banner extends FrameLayout implements OnPageChangeListener {
    ...
    public Banner start(long delayMillis) {
        setBannerStyleUI();
    ...
    private void setData(long delayMillis) {
        currentItem = 1;
        if (adapter == null) {
    ...
    public void startAutoPlay(long delayMillis) {
        ...
        handler.postDelayed(task, delayMillis > 0 ? delayMillis : delayTime);
    }
    ...
    private final Runnable task = new Runnable() {
        @Override
        public void run() {
//            Log.d("banner", "autoPlay..,,count::" + count + ",,currentItem:::" + currentItem);
            if (count > 1 && isAutoPlay) {
//                Log.i(tag, "curr:" + currentItem + " count:" + count);
                if (currentItem == 1) {
                    if (MotionEventCurrent == 0) {
                ...
                MotionEventCurrent = 0;
                currentItem = currentItem % (count + 1) + 1;
            }
        }
    };

    private int MotionEventCurrent;
