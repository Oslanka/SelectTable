package com.cloud.lashou.widget.indicator;
/**
...
public class PagerSlidingTabStripMsg extends HorizontalScrollView {


    private List<Integer> msgNum = new ArrayList<Integer>();

    public List<Integer> getMsgNum() {
        return msgNum;
    }

    public void setMsgNum(List<Integer> msgNum) {
        this.msgNum.clear();
        this.msgNum.addAll(msgNum);
    }
    ...
    public PagerSlidingTabStripMsg(Context context) {
        this(context, null);
    }

    public PagerSlidingTabStripMsg(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PagerSlidingTabStripMsg(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void addTextTab(final int position, String title) {
        View layoutView = View.inflate(getContext(), R.layout.layout_title_item, null);
    ...
    public void setMsgToast(int msgPosition, boolean isMsg) {
        ...
        if (v != null) {
            if (isMsg) {
                if (msgNum.size() > msgPosition) {
                    BadgeView msg = (BadgeView) v.findViewById(R.id.iv_msg);
                    msg.setVisibility(VISIBLE);
                    msg.setBackground(12, Color.parseColor("#F65B71"));
                    msg.setBadgeCount(msgNum.get(msgPosition));
                }
            } else {
    ...
    public void setUnderBitmap(Bitmap mUnderBitmap) {
        this.mUnderBitmap = mUnderBitmap;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float lineWidth, scorllX;

        lineWidth = lineRight - lineLeft;
        if (lineWidth > 74) {
            scorllX = (lineWidth - 74) / 2;
        } else {
            scorllX = 0;
        }
        ...
        Rect mRectfirst = new Rect(0, 0, mUnderBitmap.getWidth(), mUnderBitmap.getHeight());

        if (indicatorinFollowerTv) {
            Rect mRect = new Rect((int) (lineLeft + (lineRight - lineLeft - tvWidth) / 2), (int) (height - indicatorHeight), (int) (lineLeft + tvWidth + (lineRight - lineLeft - tvWidth) / 2), height);
            ...
            canvas.drawBitmap(mUnderBitmap, mRectfirst, mRect, null);
        } else {
            canvas.drawRect(lineLeft + scorllX, height - indicatorHeight, lineRight - scorllX, height, rectPaint);
            Rect mRect = new Rect((int) (lineLeft + scorllX), (int) (height - indicatorHeight), (int) (lineRight - scorllX), height);
            canvas.drawBitmap(mUnderBitmap, mRectfirst, mRect, null);
        }
